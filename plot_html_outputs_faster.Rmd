---
title: "`r paste('Atlantis Output Plots - version', gsub('/','',sub('.*AtlantisCalCurr', '', dir_version)))`"
output:
  html_document:
    toc: yes
    toc_float: yes
...
fl <- here::here(dir_version,"CalCurrentV3_utm_testvertmix.bgm")
# load the file
bgm <- read_bgm(fl)
# names(bgm)
emocc_sf <- box_sf(bgm)
...
grps <- read_csv(here::here(dir_version,groups_csv)) %>% mutate(Code=GroupType) %>% mutate(GroupType=InvertType)
...
plankton_groups <- grps %>% 
  filter(GroupType %in% c("PWN",'CEP','LG_ZOO','MED_ZOO','SM_ZOO','LG_PHY','SM_PHY')) %>% 
  mutate(BiomassType="plankton")
bottom_groups <- grps %>% 
  filter(GroupType %in% c("MOB_EP_OTHER",'SED_EP_FF','SED_EP_OTHER','SEAGRASS','PHYTOBEN')) %>% 
  mutate(BiomassType="2D")
other_groups <- grps %>% 
  filter(GroupType %in% c("LG_INF","MICROPHTYBENTHOS","SED_BACT","PL_BACT","SM_INF","CARRION","LAB_DET","REF_DET"))%>% 
...
tic("Loading files: ")
# output file
out_fl <- here::here(dir_version, "outputFolder", "outputCCV4_test.nc")
out <- tidync(out_fl)
...
# diets table
diet_check <- read.table(here::here(dir_version, "outputFolder", "outputCCV4_testDietCheck.txt"), as.is = TRUE,header=TRUE,sep=" ")
toc()
```
