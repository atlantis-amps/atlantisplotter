...
st_crs(emocc_sf) <- sf::st_crs(attr(emocc_sf$geometry, "crs")$proj)
...
# Actually pull the data from the .nc
  ...
  abun2 <- abun1 %>%
    ...
    dplyr::mutate(age=parse_number(age_group))
...
plot_wage_timeseries <- function(fg){
  ...
  rnsn_summ <- purrr::map2(rnsn,relnums,`*`) %>% 
    ...
    mutate(t=tyrs) %>%
    # pivot to long form
    tidyr::pivot_longer(cols = contains(fg_atts$Name),names_to = 'age_group',values_to = 'totN') %>%
    dplyr::mutate(age=parse_number(age_group)) %>% 
...
plot_Diets<-function(dietsAll, FG_to_plot, threshold=0, yearsselected){
...
thisdietdata <- subDiet %>%
  ...
  left_join(fgrps, by = c("variable" = "Code"))
...
```{r,eval=F}
plot_Preds<-function(dietsAll, FG_to_plot, threshold=0, yearsselected){
...
diet_plots <- purrr::map(diets_to_plot, plot_Diets, dietsAll=diet_check, threshold=0.9, yearsselected="all")
tictoc::toc()
```
